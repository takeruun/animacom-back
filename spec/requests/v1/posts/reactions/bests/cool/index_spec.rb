...
RSpec.describe "V1::Posts::Reactions::Bests::Cool::Index", type: :request do
  describe "GET /" do
    ...
    before do
      ...
      offsets.each.with_index do |offset, index|
        users.shift(offset).each do |user|
          create(:reaction, user: user, kind: 'cool', post: posts[index])
        end
      end
    end

    shared_examples 'かっこいいされている5件取得できる' do
      it do
        if defined?(auth_tokens)
          get '/v1/posts/reactions/bests/cool', headers: auth_tokens
        else
          get '/v1/posts/reactions/bests/cool'
        end
    ...
    context 'ログイン時' do
      ...
      it_behaves_like 'かっこいいされている5件取得できる'
