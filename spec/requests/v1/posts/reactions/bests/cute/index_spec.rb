...
RSpec.describe "V1::Posts::Reactions::Bests::Cute::Index", type: :request do
  describe "GET /" do
    ...
    before do
      ...
      offsets.each.with_index do |offset, index|
        users.shift(offset).each do |user|
          create(:reaction, user: user, kind: 'cute', post: posts[index])
        end
      end
    end

    shared_examples 'かわいいされている5件取得できる' do
      it do
        if defined?(auth_tokens)
          get '/v1/posts/reactions/bests/cute', headers: auth_tokens
        else
          get '/v1/posts/reactions/bests/cute'
        end
    ...
    context 'ログイン時' do
      ...
      it_behaves_like 'かわいいされている5件取得できる'
